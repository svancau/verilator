...
Ensure that the header-only libraries are present in the repository
    git submodule update --init --recursive --remote

If you plan to use VHDL support, you will need to install nvc simulator.
Nvc is used as a VHDL frontend for Verilator.

   git clone https://github.com/svancau/nvc.git # Clone nvc

Follow nvc's readme file to install it and ensure nvc is in your $PATH.
Currently, you'll need to use the dump_json branch.
